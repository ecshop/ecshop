...
use App\Api\Admin\Requests\ArticleCat\ArticleCatUpdateRequest;
...
{
    #[OA\Post(path: '/articleCat/query', summary: '查询列表接口', security: [['bearerAuth' => []]], tags: ['模块'])]
    ...
    #[OA\Post(path: '/articleCat/store', summary: '新增接口', security: [['bearerAuth' => []]], tags: ['模块'])]
    ...
    public function store(ArticleCatCreateRequest $createRequest): JsonResponse
    {
        ...
        try {
            ...
            $articleCatService = new ArticleCatService;
            if ($articleCatService->save($input->toEntity())) {
                DB::commit();
    ...
    #[OA\Get(path: '/articleCat/show', summary: '获取详情接口', security: [['bearerAuth' => []]], tags: ['模块'])]
    ...
    public function show(Request $request): JsonResponse
    {
        ...
        try {
            $articleCatService = new ArticleCatService;
            $articleCat = $articleCatService->getOneById($id);
            if (empty($articleCat)) {
    ...
    #[OA\Put(path: '/articleCat/update', summary: '更新接口', security: [['bearerAuth' => []]], tags: ['模块'])]
    #[OA\Parameter(name: 'id', description: 'ID', in: 'query', required: true, example: 1)]
    ...
    public function update(ArticleCatUpdateRequest $updateRequest): JsonResponse
    {
        ...
        try {
            ...
            $articleCatService->updateById($input->toEntity(), $id);
    ...
    #[OA\Delete(path: '/articleCat/destroy', summary: '删除接口', security: [['bearerAuth' => []]], tags: ['模块'])]
    ...
    public function destroy(ArticleCatDestroyRequest $destroyRequest): JsonResponse
    {
        ...
        try {
            $articleCatService = new ArticleCatService;
            if ($articleCatService->removeByIds($requestData['ids'])) {
                DB::commit();
