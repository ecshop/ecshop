...
use App\Api\Admin\Requests\Article\ArticleUpdateRequest;
...
{
    #[OA\Post(path: '/article/query', summary: '查询列表接口', security: [['bearerAuth' => []]], tags: ['模块'])]
    ...
    #[OA\Post(path: '/article/store', summary: '新增接口', security: [['bearerAuth' => []]], tags: ['模块'])]
    ...
    public function store(ArticleCreateRequest $createRequest): JsonResponse
    {
        ...
        try {
            ...
            $articleService = new ArticleService;
            if ($articleService->save($input->toEntity())) {
                DB::commit();
    ...
    #[OA\Get(path: '/article/show', summary: '获取详情接口', security: [['bearerAuth' => []]], tags: ['模块'])]
    ...
    #[OA\Put(path: '/article/update', summary: '更新接口', security: [['bearerAuth' => []]], tags: ['模块'])]
    #[OA\Parameter(name: 'id', description: 'ID', in: 'query', required: true, example: 1)]
    ...
    public function update(ArticleUpdateRequest $updateRequest): JsonResponse
    {
        ...
        try {
            ...
            $articleService->updateById($input->toEntity(), $id);
    ...
    #[OA\Delete(path: '/article/destroy', summary: '删除接口', security: [['bearerAuth' => []]], tags: ['模块'])]
    ...
    public function destroy(ArticleDestroyRequest $destroyRequest): JsonResponse
    {
        ...
        try {
            $articleService = new ArticleService;
            if ($articleService->removeByIds($requestData['ids'])) {
                DB::commit();
