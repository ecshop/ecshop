...
use App\Api\Admin\Requests\UserRank\UserRankUpdateRequest;
...
{
    #[OA\Post(path: '/userRank/query', summary: '查询列表接口', security: [['bearerAuth' => []]], tags: ['模块'])]
    ...
    #[OA\Post(path: '/userRank/store', summary: '新增接口', security: [['bearerAuth' => []]], tags: ['模块'])]
    ...
    public function store(UserRankCreateRequest $createRequest): JsonResponse
    {
        ...
        try {
            ...
            $userRankService = new UserRankService;
            if ($userRankService->save($input->toEntity())) {
                DB::commit();
    ...
    #[OA\Get(path: '/userRank/show', summary: '获取详情接口', security: [['bearerAuth' => []]], tags: ['模块'])]
    ...
    public function show(Request $request): JsonResponse
    {
        ...
        try {
            $userRankService = new UserRankService;
            $userRank = $userRankService->getOneById($id);
            if (empty($userRank)) {
    ...
    #[OA\Put(path: '/userRank/update', summary: '更新接口', security: [['bearerAuth' => []]], tags: ['模块'])]
    #[OA\Parameter(name: 'id', description: 'ID', in: 'query', required: true, example: 1)]
    ...
    public function update(UserRankUpdateRequest $updateRequest): JsonResponse
    {
        ...
        try {
            ...
            $userRankService->updateById($input->toEntity(), $id);
    ...
    #[OA\Delete(path: '/userRank/destroy', summary: '删除接口', security: [['bearerAuth' => []]], tags: ['模块'])]
    ...
    public function destroy(UserRankDestroyRequest $destroyRequest): JsonResponse
    {
        ...
        try {
            $userRankService = new UserRankService;
            if ($userRankService->removeByIds($requestData['ids'])) {
                DB::commit();
