...
class exchange
{
     ...
     * @return void
     */
    private function __construct($table, $db, $id, $name)
    {
        $this->table = $table;
        $this->db = $db;
        $this->id = $id;
     ...
     * @return void
     */
    private function is_only($col, $name, $id = 0, $where = '')
    {
     ...
     * @return   int        记录个数
     */
    private function num($col, $name, $id = 0)
    {
        $sql = 'SELECT COUNT(*) FROM '.$this->table." WHERE $col = '$name'";
        $sql .= empty($id) ? '' : ' AND '.$this->id." != '$id' ";
     ...
     * @return string   取出的数据
     */
    private function get_name($id, $name = '')
    {
        if (empty($name)) {
            $name = $this->name;
        }
