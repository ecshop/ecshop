...
<table width="100%" cellpadding="3" cellspacing="1" >
  ...
  <tr>
    ...
    <td colspan="3">{!! nl2br(e($booking['goods_desc'])) !!}</td>
  </tr>
  <tr>
    <td align="right" class="first-cell" >{{ $lang['link_man'] }}：</td>
    <td>{{ $booking['link_man'] }} @if($booking['email'])<{{ $booking['email'] }}>@endif </td>
...
<form name='theForm' method="post" action="goods_booking.php" onsubmit="return validate()">
  ...
  @if($booking['is_dispose'])<input type="submit" name="remail" value="{{ $lang['remail'] }}" class="button">@endif
