...
<table cellpadding="3" cellspacing="1">
  ...
  <tr>
    ...
    <td>@if($comment['user_name']){{ $comment['user_name'] }}@else{{ $lang['anonymous'] }}@endif</td>
    <td>{{ $lang['type[$comment']['comment_type]'] }}</td>
    <td><a href="../@if($comment['comment_type'] == '0')goods@elsearticle@endif.php?id={{ $comment['id_value'] }}" target="_blank">{{ $comment['title'] }}</td>
    <td>{{ $comment['ip_address'] }}</td>
    <td align="center">{{ $comment['add_time'] }}</td>
    <td align="center">@if($comment['status'] == 0){{ $lang['hidden'] }}@else{{ $lang['display'] }}@endif</td>
    <td align="center">
