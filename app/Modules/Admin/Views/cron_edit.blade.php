...
<table cellspacing="1" cellpadding="3" width="100%">
  ...
  <tr>
    ...
    <td>
      <input name="cron_name" type="text" value="{{ $cron['cron_name'] }}" size="40" readonly="readonly" style="background:#dddddd" />
    </td>
  </tr>
  <tr>
    <td class="label">{{ $lang['cron_desc'] }}</td>
    <td><textarea name="cron_desc" cols="80" rows="5" style="" readonly="readonly" style="background:#dddddd" >{{ $cron['cron_desc'] }}</textarea></td>
  </tr>
  @foreach($cron['cron_config'] as $config)
  <tr>
    ...
    <td><select name="cfg_value[]">@foreach($config['range'] as $__k => $__v)<option value="{{ $__k }}" @if($__k == $config['value']) selected @endif>{{ $__v }}</option>@endforeach</select></td>
    @endif
  ...
  <tr>
    ...
    <td>
      ...
      <select name="cron_day" id="cron_day" disabled="true">@foreach($days as $__k => $__v)<option value="{{ $__k }}" @if($__k == $cron['cronday']) selected @endif>{{ $__v }}</option>@endforeach</select>&nbsp;{{ $lang['cron_day'] }}&nbsp;
      <input type="radio" name="ttype" id="ttype_week" value="week" onClick="dotype('week');"/>{{ $lang['cron_week'] }}
      <select name="cron_week" id="cron_week" disabled="true">@foreach($week as $__k => $__v)<option value="{{ $__k }}" @if($__k == $cron['cronweek']) selected @endif>{{ $__v }}</option>@endforeach</select>
      <input type="radio" name="ttype" id="ttype_unlimit" value="unlimit" onClick="dotype('unlimit')">{{ $lang['cron_unlimit'] }}
      <br />
      {{ $lang['cron_thatday'] }}<select name="cron_hour">@foreach($hours as $__k => $__v)<option value="{{ $__k }}" @if($__k == $cron['cronhour']) selected @endif>{{ $__v }}</option>@endforeach</select>&nbsp;{{ $lang['cron_hour'] }}&nbsp;</td>
  </tr>
  <tr>
  ...
  <tr>
    <td></td>
    <td>
<span class="notice-span" @if($help_open)style="display:block" @else style="display:none" @endif id="notice_minute">{{ $lang['notice_minute'] }}</span></td>
  </tr>
  <tr>
  ...
  <tr id="advance_2">
    <td></td>
    <td>
<span class="notice-span" @if($help_open)style="display:block" @else style="display:none" @endif id="notice_alow_ip">{{ $lang['notice_alow_ip'] }}</span></td>
  </tr>
  <tr id="advance_3">
    ...
    <td>
    @foreach($page_list as $page_list => $list)
    <div style="width:200px;float:left;">
    <label><input type="checkbox" name="alow_files[]" value="{{ $page_list }}.php" id="{{ $page_list }}" class="checkbox" @if($list == 1) checked="true" @endif />
  ...
  <tr id="advance_4">
    <td></td>
    <td>
<span class="notice-span" @if($help_open)style="display:block" @else style="display:none" @endif id="notice_alow_files">{{ $lang['notice_alow_files'] }}</span></td>
  </tr>
  <tr align="center">
    <td colspan="2">
