...
<table id="group-table" cellspacing="1" cellpadding="3" width="100%">
  ...
  <tr>
    ...
    <td><input name="deposit" type="text" id="deposit" value="{{ $group_buy['deposit'] ?? 0 }}" size="30"></td>
  </tr>
  <tr>
    ...
    <td><input type="text" name="restrict_amount" value="{{ $group_buy['restrict_amount'] ?? 0 }}" size="30" />
      <br /><span class="notice-span" @if($help_open)style="display:block" @else style="display:none" @endif id="noticRestrict">{{ $lang['notice_restrict_amount'] }}</span>    </td>
  </tr>
  <tr>
    <td class="label">{{ $lang['label_gift_integral'] }}</td>
    <td><input type="text" name="gift_integral" value="{{ $group_buy['gift_integral'] ?? 0 }}" size="30" /></td>
  </tr>
  @foreach($group_buy['price_ladder'] as $key => $item)
  @if($key == 0)
  <tr>
