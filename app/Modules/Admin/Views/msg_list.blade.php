...
<table cellpadding="3" cellspacing="1">
  ...
  <tr>
    ...
    <td align="left">{{ \Illuminate\Support\Str::limit($msg['msg_title'], 40, '...') }}</td>
    <td align="center">{{ $msg['msg_type'] }}@if($msg['order_id'])<br><a href="order.php?act=info&order_id={{ $msg['order_id'] }}">{{ $msg['order_sn'] }}@endif</a></td>
    <td align="center"  nowrap="nowrap">{{ $msg['msg_time'] }}</td>
    @if($msg['msg_area'] == 0)
    <td align="center">{{ $lang['display'] }}</td>
    @else
    <td align="center">@if($msg['msg_status'] == 0){{ $lang['hidden'] }}@else{{ $lang['display'] }}@endif</td>
    @endif
    <td align="center">@if($msg['reply'] == 0){{ $lang['unreplyed'] }}@else{{ $lang['replyed'] }}@endif</td>
    <td align="center">
