...
<tr>
    <td>{{ $lang['system_main'] }}</td> <td>
        <select onchange="add_main(this.value);" name="menulist" id="menulist">
            <option value='-'>-</option>
            @foreach($sysmain as $key => $val)
                <option value='{{ $key }}'>@if($val[2]){{ $val[2] }}@else{{ $val[0] }}@endif</option>
            @endforeach
        </select>
    </td>
</tr>
...
<tr>
    <td><a href="javascript:showNotice('notice_url');" title="{{ $lang['notice_url'] }}"><img src="images/notice.gif" width="16" height="16" border="0" alt="{{ $lang['form_notice'] }}">{{ $lang['item_url'] }}</a></td> <td><input type="text" name="item_url" value="{{ $rt['item_url'] }}" id="item_url" size="40" onKeyPress="javascript:key();" /></td>
</tr>
  <tr>
    <td></td>
    <td>
<span class="notice-span" @if($help_open)style="display:block" @else style="display:none" @endif id="notice_url">{{ $lang['notice_url'] }}</span></td>
  </tr>
<tr>
