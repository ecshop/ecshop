...
<table cellspacing="1" cellpadding="3" width="100%">
  <tr>
    ...
    <td><input type="text" name="pack_name" maxlength="60" size = "30" value="{{ $pack['pack_name'] }}" />{{ $lang['require_field'] }}</td>
  </tr>
  <tr>
    ...
    <br /><span class="notice-span" @if($help_open)style="display:block" @else style="display:none" @endif id="notice_packfee">{{ $lang['notice_packfee'] }}</span>
    </td>
  </tr>
  <tr>
    ...
    <br /><span class="notice-span" @if($help_open)style="display:block" @else style="display:none" @endif id="notice_freemoney">{{ $lang['notice_freemoney'] }}</span>
    </td>
  </tr>
  <tr>
    <td class="label">@if($pack['pack_img'] != "")<a href="javascript:showNotice('warn_packimg');" title="{{ $lang['form_notice'] }}">
        <img src="images/warning_small.gif" width="16" height="16" border="0" alt="{{ $lang['form_notice'] }}"></a>@endif{{ $lang['pack_img'] }}</td>
    <td><input type="file" name="pack_img"  size="45">@if($pack['pack_img'] != "")<input type="button" value="{{ $lang['drop_pack_img'] }}" onclick="if (confirm('{{ $lang['confirm_pack_img'] }}'))location.href='pack.php?act=drop_pack_img&id={{ $pack['pack_id'] }}'">@endif
    <br /><span class="notice-span" @if($help_open)style="display:block" @else style="display:none" @endif id="warn_packimg">{{ $lang['warn_packimg'] }}</span>
    </td>
  </tr>
  <tr>
