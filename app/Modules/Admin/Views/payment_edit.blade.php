...
<table cellspacing="1" cellpadding="3" width="100%">
  ...
  @foreach($pay['pay_config'] as $key => $config)
  <tr>
    <td class="label">
      @if($config['desc'])
    ...
    <td>
      ...
      <select name="cfg_value[]">@foreach($config['range'] as $__k => $__v)<option value="{{ $__k }}" @if($__k == $config['value']) selected @endif>{{ $__v }}</option>@endforeach</select>
      @endif
      ...
      @if($config['desc'])
      <br /><span class="notice-span" @if($help_open)style="display:block" @else style="display:none" @endif id="notice{{ $config['name'] }}">{{ $config['desc'] }}</span>
      @endif
    </td>
  </tr>
  @endforeach
  <tr>
    ...
    <td>@if($pay['is_cod'] )<input name="pay_fee" type="hidden" value="{{ $pay['pay_fee'] ?? 0 }}" />{{ $lang['decide_by_ship'] }}
        @else<input name="pay_fee" type="text" value="{{ $pay['pay_fee'] ?? 0 }}" />@endif
    </td>

  </tr>
  <tr>
    <td class="label">{{ $lang['payment_is_cod'] }}</td>
    <td>@if($pay['is_cod'] == "1"){{ $lang['yes'] }}@else{{ $lang['no'] }}@endif</td>
  </tr>
  <tr>
    <td class="label">{{ $lang['payment_is_online'] }}</td>
    <td>@if($pay['is_online'] == "1"){{ $lang['yes'] }}@else{{ $lang['no'] }}@endif</td>
  </tr>
  <tr align="center">
    <td colspan="2">
