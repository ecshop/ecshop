      ...
      <td>
         <select name="shop_country" id="selCountries" onchange="region.changed(this, 1, 'selProvinces')">
           ...
           @foreach($countries as $region)
             <option value="{{ $region['region_id'] }}" @if($region['region_id'] == 1)selected@endif>{{ $region['region_name'] }}</option>
           @endforeach
         </select>                
       </td>
    </tr>
         ...
         <select name="shop_province" id="selProvinces" onchange="region.changed(this, 2, 'selCities')">
           <option value=''>{{ $lang['select_please'] }}</option>
             @foreach($provinces as $region)
               <option value="{{ $region['region_id'] }}" @if($region['region_id'] == 2)selected@endif> {{ $region['region_name'] }}</option>
             @endforeach
         </select>
      </td>
    </tr>
        ...
        <select name="shop_city" id="selCities">
          <option value=''>{{ $lang['select_please'] }}</option>
            @foreach($cities as $region)
              <option value="{{ $region['region_id'] }}" @if($region['region_id'] == 35)selected@endif>{{ $region['region_name'] }}</option>
            @endforeach
        </select>
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
          ...
          <option value=''>{{ $lang['select_please'] }}</option>
            ...
            @foreach($modules_payment as $module)
