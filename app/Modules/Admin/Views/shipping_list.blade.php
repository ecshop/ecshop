...
<table cellspacing='1' cellpadding='3'>
  ...
  <tr>
    ...
    <td align='center'>@if($module['cod']==1){{ $lang['yes'] }}@else{{ $lang['no'] }}@endif</td>
    <td nowrap="true">{{ $module['version'] }}</td>
    <td nowrap="true"><a href="{{ $module['website'] }}" target="_blank">{{ $module['author'] }}</a></td>
    <td align="right" valign="top"> @if($module['install'] == 1) <span onclick="listTable.edit(this, 'edit_order', '{{ $module['code'] }}'); return false;">{{ $module['shipping_order'] }}</span> @else &nbsp; @endif </td>
