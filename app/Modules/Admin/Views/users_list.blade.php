...
<div class="form-div">
  <form action="javascript:searchUser()" name="searchForm">
    ...
    &nbsp;{{ $lang['label_rank_name'] }} <select name="user_rank"><option value="0">{{ $lang['all_option'] }}</option>@forelse($user_ranks as $__k => $__v)<option value="{{ $__k }}">{{ $__v }}</option>@endforeach</select>
...
<table cellpadding="3" cellspacing="1">
  ...
  <tr>
    <td><input type="checkbox" name="checkboxes[]" value="{{ $user['user_id'] }}" notice="@if($user['user_money'] ne 0)1@else0@endif"/>{{ $user['user_id'] }}</td>
    <td class="first-cell">{{ $user['user_name'] }}</td>
    <td><span onclick="listTable.edit(this, 'edit_email', {{ $user['user_id'] }})">{{ $user['email'] }}</span></td>
    <td align="center">@if($user['is_validated']) <img src="images/yes.gif"> @else <img src="images/no.gif"> @endif</td>
    ...
    <td align="center">
      ...
      <a href="javascript:confirm_redirect('@if($user['user_money'] ne 0){{ $lang['still_accounts'] }}@endif{{ $lang['remove_confirm'] }}', 'users.php?act=remove&id={{ $user['user_id'] }}')" title="{{ $lang['remove'] }}"><img src="images/icon_drop.gif" border="0" height="16" width="16" /></a>
    </td>
  </tr>
  @empty
