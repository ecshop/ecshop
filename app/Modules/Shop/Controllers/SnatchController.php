...
{
    #[OA\Get(path: 'snatch', summary: '接口', tags: ['模块'])]
    ...
    private function get_myprice($id)
    {
        ...
        if (! empty($_SESSION['user_id'])) {
            ...
            if ($my_price) {
                /* 取得用户唯一价格 */
                $sql = 'SELECT bid_price , count(*) AS num FROM '.$GLOBALS['ecs']->table('snatch_log')."  WHERE snatch_id ='$id' AND bid_price ".db_create_in(implode(',', $my_price)).' GROUP BY bid_price HAVING num = 1';
