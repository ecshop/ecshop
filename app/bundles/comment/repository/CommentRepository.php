...
namespace app\bundles\comment\repository;
...
class CommentRepository extends CurdRepository implements RepositoryInterface
{
     ...
     * 添加
     */
    public function createByEntity(CommentEntity $entity): int    {
     ...
     * 按照ID查询返回对象
     */
    public function findOneEntityById(int $id): ?CommentEntity
    {
     ...
     * 按照条件查询
     */
    public function findOneEntity(array $condition): ?CommentEntity
    {
     ...
     * 定义数据数据模型类
     */
    public function model(string $modelName = 'Comment'): Model
    {
        $model = '\\app\\bundles\\comment\\model\\'.$modelName.'Model';
