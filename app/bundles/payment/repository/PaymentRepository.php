...
namespace app\bundles\payment\repository;
...
class PaymentRepository extends CurdRepository implements RepositoryInterface
{
     ...
     * 添加
     */
    public function createByEntity(PaymentEntity $entity): int    {
     ...
     * 按照ID查询返回对象
     */
    public function findOneEntityById(int $id): ?PaymentEntity
    {
     ...
     * 按照条件查询
     */
    public function findOneEntity(array $condition): ?PaymentEntity
    {
     ...
     * 定义数据数据模型类
     */
    public function model(string $modelName = 'Payment'): Model
    {
        $model = '\\app\\bundles\\payment\\model\\'.$modelName.'Model';
