...
class MagazineListController extends InitController
{
    ...
    public function addAction()
    {
        if (empty($_POST['step'])) {
            ...
            $time = gmtime();
            $sql = "INSERT INTO " . table('mail_templates') . " (template_code, is_html,template_subject, template_content, last_modify, type) VALUES('" . md5($magazine_name . $time) . "',1, '$magazine_name', '$magazine_content', '$time', 'magazine')";
            $db->query($sql);
    ...
    public function editAction()
    {
        ...
        if (empty($_POST['step'])) {
            $rt = $db->getRow("SELECT * FROM " . table('mail_templates') . " WHERE type = 'magazine' AND template_id = '$id'");
            ...
            $magazine_content = str_replace('src=\"', 'src=\"http://' . $_SERVER['HTTP_HOST'], $magazine_content);
            $time = gmtime();
            $db->query("UPDATE " . table('mail_templates') . " SET is_html = 1, template_subject = '$magazine_name', template_content = '$magazine_content', last_modify = '$time' WHERE type = 'magazine' AND template_id = '$id'");
            $links[] = array('text' => $_LANG['magazine_list'], 'href' => 'magazine_list.php?act=list');
            sys_msg($_LANG['edit_ok'], 0, $links);
        }
    }
    ...
    {
        $id = intval($_REQUEST['id']);
        $db->query("DELETE  FROM " . table('mail_templates') . " WHERE type = 'magazine' AND template_id = '$id' LIMIT 1");
        $links[] = array('text' => $_LANG['magazine_list'], 'href' => 'magazine_list.php?act=list');
        sys_msg($_LANG['edit_ok'], 0, $links);
    }
    ...
    public function addtolistAction()
    {
        ...
        $template_id = $db->getOne("SELECT template_id FROM " . table('mail_templates') . " WHERE type = 'magazine' AND template_id = '$id'");
        if (!empty($template_id)) {
            if (SEND_LIST == $rank_array['0']) {
                $count = $db->getOne("SELECT COUNT(*) FROM " . table('email_list') . "WHERE stat = 1");
                if ($count > $start) {
                    $sql = "SELECT email FROM " . table('email_list') . "WHERE stat = 1 LIMIT $start,100";
                    $query = $db->query($sql);
                    $add = '';
                    ...
                    if ($add) {
                        $sql = "INSERT INTO " . table('email_sendlist') . " (email,template_id,pri,last_send) VALUES " . $add;
                        $db->query($sql);
                    }
                    if ($i == 100) {
                        $start = $start + 100;
                    ...
                    $db->query("UPDATE " . table('mail_templates') . " SET last_send = " . time() . " WHERE type = 'magazine' AND template_id = '$id'");
                    $links[] = array('text' => $_LANG['magazine_list'], 'href' => 'magazine_list.php?act=list');
                    sys_msg($_LANG['edit_ok'], 0, $links);
                }
            } else {
                $sql = "SELECT special_rank FROM " . table('user_rank') . " WHERE rank_id = '" . $rank_array['1'] . "'";
                $row = $db->getRow($sql);
                if (SEND_USER == $rank_array['0']) {
                    $count_sql = 'SELECT COUNT(*) FROM ' . table('users') . 'WHERE is_validated = 1';
                    $email_sql = 'SELECT email FROM ' . table('users') . "WHERE is_validated = 1 LIMIT $start,100";
                } elseif ($row['special_rank']) {
                    $count_sql = 'SELECT COUNT(*) FROM ' . table('users') . 'WHERE is_validated = 1 AND user_rank = ' . $rank_array['1'];
                    $email_sql = 'SELECT email FROM ' . table('users') . 'WHERE is_validated = 1 AND user_rank = ' . $rank_array['1'] . " LIMIT $start,100";
                } else {
                    $count_sql = 'SELECT COUNT(*) ' .
                        'FROM ' . table('users') . ' AS u LEFT JOIN ' . table('user_rank') . ' AS ur ' .
    ...
    public function get_magazine()
    {
        ...
        if ($result === false) {
            ...
            $sql = "SELECT count(*) FROM " . table('mail_templates') . " WHERE type = 'magazine'";
