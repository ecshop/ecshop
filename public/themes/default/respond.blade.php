      ...
      <p style="font-size: 14px; font-weight:bold; color: red;">{{ $message }}</p>
        ...
        @foreach($virtual_card as $vgoods)
          @foreach($vgoods['info'] as $card)
            ...
            <td bgcolor="#FFFFFF">
            @if($card['card_sn'])<strong>{{ $lang['card_sn'] }}:</strong>{{ $card['card_sn'] }}@endif
            @if($card['card_password'])<strong>{{ $lang['card_password'] }}:</strong>{{ $card['card_password'] }}@endif
            @if($card['end_date'])<strong>{{ $lang['end_date'] }}:</strong>{{ $card['end_date'] }}@endif
            </td>
            </tr>
          @endforeach
        @endforeach
