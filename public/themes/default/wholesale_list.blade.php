...
<div class="block">
  ...
  <div class="blank"></div>
    <table width="100%" border="0" cellpadding="5" cellspacing="1" bgcolor="#dddddd">
          ...
          <!-- @foreach($cart_goods as $key => $goods) 循环批发商品开始 -->
          <tr>
...
<div class="block">
  ...
  <a href="javascript:void(0);" onClick="javascript:display_mode_wholesale('list')"><img align="middle" src="images/display_mode_list@if($pager['display'] == 'list')_act@endif.gif" alt="{{ $lang['display']['list'] }}"></a>
  <a href="javascript:void(0);" onClick="javascript:display_mode_wholesale('text')"><img align="middle" src="images/display_mode_text@if($pager['display'] == 'text')_act@endif.gif" alt="{{ $lang['display']['text'] }}"></a>&nbsp;&nbsp;<a href="wholesale.php?act=price_list" class="f6">{{ $lang['ws_price_list'] }}</a></h5>
  <div class="blank"></div>
  ...
  <table border="0" cellpadding="5" cellspacing="1" width="100%">
    <form method="post" action="wholesale.php?act=list" name="wholesale_search">
      <tr>
        ...
        <input name="search_keywords" type="text" id="search_keywords" value="{{ $search_keywords }}" style="width:110px;"/>
  ...
  <form name="wholesale_goods" action="wholesale.php?act=add_to_cart" method="post">
          <table width="100%" border="0" cellpadding="5" cellspacing="1" bgcolor="#dddddd">
            ...
            <!-- @foreach($wholesale_list as $wholesale) 循环批发商品开始 -->
            <tr>
              <td bgcolor="#ffffff">@if($pager['display'] == 'list')<a href="{{ $wholesale['goods_url'] }}" target="_blank"><img src="{{ $wholesale['goods_thumb'] }}" alt="{{ $wholesale['goods_name'] }}" /></a>@endif<a href="{{ $wholesale['goods_url'] }}" target="_blank" class="f6">{{ $wholesale['goods_name'] }}</a></td>
              <td bgcolor="#ffffff">

                <table width="100%" border="0" align="center">
                  @foreach($wholesale['goods_attr'] as $key => $property_group)
                  @foreach($property_group as $property)
                  <tr>
                    <td nowrap="true" style="border-bottom:2px solid #ccc;">{{ $property['name'] }}</td>
                  ...
                  @if($attr_price['attr'] != '')
                   <tr>
                    <td align="left" nowrap="true" bgcolor="#ffffff" style="padding:5px;" colspan="2">
                      @foreach($attr_price['attr'] as $attr_key => $attr_value)                {{ $attr_value['attr_name'] }}:{{ $attr_value['attr_val'] }}&nbsp;@endforeach
                    </td>
                  </tr>
                  @endif
